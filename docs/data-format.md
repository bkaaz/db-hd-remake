...
- **`transitions[]`** — evaluated **in order, first match wins**, and **at most
  ...
  - `when` — trigger, optionally negated with a leading `!`. v0 vocabulary:
    ...
    - `pressed:punch` / `pressed:kick` — that attack button went down **this
      frame** (an edge, so holding the key does not repeat the move). One
      trigger per button rather than a generic `pressed:attack`, which stopped
      meaning anything once there was more than one.
